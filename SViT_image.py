...
SCATTER_LAYER = 2
scattering = Scattering2D(J=SCATTER_LAYER, L=8, shape=(IMAGE_SIZE, IMAGE_SIZE), max_order=2)
...
def evaluate(model, data_loader, loss_history, acc_history):
    ...
    acc = correct_samples.item() / total_samples
    ...
    print('\nAverage test loss: ' + '{:.4f}'.format(avg_loss) +
          ...
          '{:4.2f}'.format(100.0 * correct_samples.item() / total_samples) + '%)\n')

start_time = time.time()
SCALE = pow(2,SCATTER_LAYER)
# print(PATCH_SIZE // SCALE)
model = ViT(image_size=IMAGE_SIZE // SCALE, patch_size=2, num_classes=NUM_CLASS, channels=3*K,
...
scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=7, verbose=True, min_lr=1e-3*1e-5, factor=0.1)

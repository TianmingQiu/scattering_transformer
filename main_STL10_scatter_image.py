...
scattering = Scattering2D(J=1, L=6, shape=(96, 96))
...
def train_epoch(model, optimizer, data_loader, loss_history):
    ...
    for i, (data, target) in enumerate(data_loader):
        ...
        data = rearrange(scattered_data, 'b c x h d -> b (c x) h d')
        optimizer.zero_grad()
...
model = ViT(image_size=96 // 2, patch_size=8 // 2, num_classes=10, channels=3 * 7,
